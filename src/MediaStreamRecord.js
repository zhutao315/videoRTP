...
function WhammyRecorderHelper(mediaStream, root) {
  ...
  function isBlankFrame(frame, _pixTolerance, _frameTolerance) {
    ...
    const maxColorDifference = Math.sqrt(Math.pow(255, 2) + Math.pow(255, 2) + Math.pow(255, 2))
  ...
  function dropBlackFrames(_frames, _framesToCheck, _pixTolerance, _frameTolerance) {
    ...
    const endCheckFrame = _framesToCheck && _framesToCheck > 0 && _framesToCheck <= _frames.length ? _framesToCheck : _frames.length
    const sampleColor = {
      r: 0,
      g: 0,
      b: 0,
